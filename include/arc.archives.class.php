    ...
    function ParAddTable()
    {
        ...
        if($this->ChannelUnit->ChannelInfos['addtable']!='')
        {
            ...
            @SetSysEnv($this->Fields['typeid'],$this->Fields['typename'],$this->Fields['id'],$this->Fields['title'],'archives');
                    //替换图片Alt为文档标题  
$this->Fields['body'] = str_ireplace(array('title="','title=\'\''),'tit="',$this->Fields['body']);
$this->Fields['body'] = str_ireplace(array('alt="','alt=\'\''),'name="',$this->Fields['body']);
//$this->Fields['body'] = preg_replace("@[\s]{0,}alt[\s]{0,}=[\"'\s]{0,}[\s\S]{0,}[\"'\s] @isU"," ",$this->Fields['body']); 
$this->Fields['body'] = str_ireplace("<img " ,"<img alt=\"".$this->Fields['title']."图片\" ",$this->Fields['body']);
//end
        }
        //完成附加表信息读取
        unset($row);
    ...
    function ReplaceKeyword($kw,&$body)
    {
        ...
        $body = preg_replace("#(<a(.*))(>)(.*)(<)(\/a>)#isU", '\\1-]-\\4-[-\\6', $body);
         //是否开启ascii编码
  	if($cfg_ascii == "Y"){
    	$this->Fields['body']=emploade($body);
}
