...
if(empty($action))
{
    ...
    $dlist->pageSize = 20;
    $dlist->SetTemplet(dirname(__FILE__)."/templets/tags_main.htm");
...
else if($action == 'update')
{
    ...
    foreach ($ids as $tid) {
		...
		$description = cn_substrR($description,1000);
        ...
        $query = "UPDATE `#@__tagindex` SET `count`='$count',`tagfenzu`='$tagfenzu',`tagimg`='$tagimg',`title`='$title',`keywords`='$keywords',`description`='$description' WHERE id='$tid' ";
        if ($dsql->ExecuteNoneQuery($query)) {
            $num += 1;
        }
    }
    ShowMsg("成功更新{$num}条标签信息!", 'tags_main.php');
    exit();
}
/*
function delete()
