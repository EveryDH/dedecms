...
class TagListView
{
	...
	function __construct($typeid,$uppage=1)
	{
		...
		if(!$this->IsError)
		{	
			$this->Fields['id'] = intval($typeid);
			$this->Fields['position'] = "";
            
            $sql = "select * from #@__tagindex  where id='$typeid' ";
            $arcRow = $dsql->GetOne($sql);
            
			$this->Fields['tag'] = trim($arcRow["tag"]);
            $this->Fields['keywords'] = $this->Fields['tag'];
            $arcRow["title"] = trim($arcRow["title"]);
            $this->Fields['title'] = empty($arcRow["title"]) ? $arcRow["tag"] : $arcRow["title"];
            $this->Fields['description'] = $arcRow['description'];
			$this->Fields['tagdir'] = trim($arcRow["tagdir"]);
            
            if (empty($this->Fields['tagdir'])) {
                $tag = $this->Fields['tag'];
                $tagdir = strtolower(GetPinyin($tag));
                $tag_row = $dsql->GetOne("SELECT * FROM `#@__tagindex` WHERE tagdir LIKE '$tagdir'");
                if (!empty($tag_row)) {
                    $tagdir = $tagdir . $this->Fields['id'];
                }
                $this->Fields['tagdir'] = $tagdir;
                $dsql->ExecuteNoneQuery("UPDATE `#@__tagindex` SET tagdir='$tagdir' WHERE tag LIKE '$tag' ");
            }
	...
	{			
		...
		//初始化列表模板，并统计页面总数
		$tempfile = $GLOBALS['cfg_basedir'].$GLOBALS['cfg_templets_dir']."/".$GLOBALS['cfg_df_style']."/taglist_m.htm";				
	...
	function MakeHtml($startpage=1,$makepagesize=0)
	{
        ...
        //TAGS目录名 默认为tag
        $tag_basedir = trim($GLOBALS['cfg_tag_basedir_m'],'/\ ');
        $tag_basedir = empty($tag_basedir) ? 'tag' : $tag_basedir;
        //TAGS目录 默认/tag/    
        $this->Fields['typedir']=$GLOBALS['cfg_cmspath'].'/'.$tag_basedir;
		...
		for($this->PageNo=$startpage; $this->PageNo < $endpage; $this->PageNo++)
		{
            ...
            //1:拼音目录    
            if ($tag_style == 1) {     
                $makeDir = $this->Fields['typedir']."/".$this->Fields['tagdir'];
			    $makeFile = $makeDir."/index_".$this->PageNo.".html";	
			    CreateDir($makeDir);
            }
            
            //2:ID目录    
            if ($tag_style == 2) {     
                $makeDir = $this->Fields['typedir']."/".$this->Fields['id'];
			    $makeFile = $makeDir."/index_".$this->PageNo.".html";	
			    CreateDir($makeDir);
            }
            ...
            if ($tag_style == 3) {     
                if ($this->Fields['tagdir'] == 'index') {
                    $this->Fields['tagdir'] .= $this->Fields['id'];
                }
			    $makeFile = $this->Fields['typedir'].'/'.$this->Fields['tagdir'].'_'.$this->PageNo.'.html';	
            }
            
            //4:ID url   如：tag/1.html   
            if ($tag_style == 4) {     
			    $makeFile = $this->Fields['typedir'].'/'.$this->Fields['id'].'_'.$this->PageNo.'.html';	
            }			
			...
			echo $this->Fields['tag'].": ". $makeFile . '<br/>';
		...
		{		
             if ($tag_style ==1 || $tag_style ==2) {     
				$list_1 = $this->GetTruePath().$makeDir."/index_1.html";						
				$indexname = $this->GetTruePath().$makeDir."/index.html";
				copy($list_1,$indexname);
                unlink($list_1);
             }
             if ($tag_style ==3 || $tag_style ==4) {  
                $makeFile = substr($makeFile,0,-1);
				$list_1 = $makeFile . '_1.html';						
				$indexname = $makeFile . '.html';  
				copy($list_1,$indexname);
                unlink($list_1);
             }              
		}
	...
	*/
	...
	function GetPageListST($list_len,$listitem="index,end,pre,next,pageno")
	{
        ...
        //2:ID目录    
        if ($tag_style == 2) { 
            $tnamerule = $this->Fields['typedir']."/".$this->Fields['id']."/index_{page}.html";        
        }
        ...
        if ($tag_style == 3) {
            ...
            $tnamerule = $this->Fields['typedir']."/".$this->Fields['tagdir']."_{page}.html"; 
         }
        ...
        for($mjj=1;$mjj<=$totalpage;$mjj++)
        {
            ...
            {    
                if ($mjj==1) {
                    $optionlist .= "<option value='".str_replace("_{page}",'',$tnamerule)."' selected>$mjj</option>\r\n";
                } else {
            ...
            {
                if ($mjj==1) {
                    $optionlist .= "<option value='".str_replace("_{page}",'',$tnamerule)."'>$mjj</option>\r\n";
                } else {
